          ...
          <tr>
            ...
            <td>
              <select
                  ...
                  @if ($data['multiply'])multiple @endif>

                  @if (isset($field['allows_null']) && $field['allows_null']==true)
                      <option value="">-</option>
                  @endif

                      @foreach ($data['parameters'] as $option)
                          @if( (old(square_brackets_to_dots($field["name"])) && in_array($option->getKey(), old($field["name"]))) || (is_null(old(square_brackets_to_dots($field["name"]))) && isset($field['value']) && in_array($option->getKey(), $field['value']->pluck($option->getKeyName(), $option->getKeyName())->toArray())))
                              <option value="{{ $option->getKey() }}" selected>{{ json_decode($option->{$field['attribute']})->ru }}</option>
                          @else
                              <option value="{{ $option->getKey() }}">{{ json_decode($option->{$field['attribute']})->ru }}</option>
                          @endif
                      @endforeach
              </select>
            </td>
